...
class Benchmark(object):
    ...
    def _init_benchmark(self) -> None:
        ...
        self._model_adaptation_cls._meta_conf._checkpoint_path = self._checkpoint_path
        # print(self._model_adaptation_cls._meta_conf)
    ...
    def _batch_size(self):
        ...
        if self._scenario.test_case.data_wise == "sample_wise":
            print("[INFO] batch size in benchmark.py:_batch_size,reture 1 of sample wise")
            return 1
        elif (
    ...
    def _online_adapt_step(
        ...
        # print("[INFO] batch size in benchmark.py:_online_adapt_step",batch._x.shape)
    ...
    def eval(self) -> Dict:
        ...
        with auxiliary.evaluation_monitor(self._meta_conf):
            ...
            # print("[INFO] batch size in benchmark.py:eval() self._test_loader.iterator",self._batch_size)
            # print("[INFO] batch size in benchmark.py:eval() self._test_loader.iterator",self._test_loader)
            for step, epoch, batch, domain_indices in self._test_loader.iterator(
                ...
                print("[INFO] batch size in benchmark.py:eval()",batch._x.shape)
        ...
        print(self._timer.summary())
        try:
            self._model_adaptation_cls.logger.close()
        except:
            pass
        return stats
