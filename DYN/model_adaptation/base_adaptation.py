...
import loads.datasets.loaders as loaders
import loads.define_dataset as define_dataset
import model_adaptation.utils as adaptation_utils
from api import Batch, Quality
from model_selection.base_selection import BaseSelection
from model_selection.group_metrics import GroupLossComputer
...
class BaseAdaptation(object):
    ...
    def _initialize_optimizer(self, params, vida: bool =False) -> torch.optim.Optimizer:
        """Set up optimizer for adaptation process."""
        if vida:
            optimizer = adaptation_utils.define_optimizer(
            self._meta_conf, params, lr=self._meta_conf.ViDALR
            )
        else:
            optimizer = adaptation_utils.define_optimizer(
    ...
    def initialize(self, seed: int):
        ...
        # print("[INFO]model info:", self._base_model.)
    ...
    def compute_fishers(self, scenario, data_size):
        ...
        for step, _, batch, _ in self.fisher_loader.iterator(
