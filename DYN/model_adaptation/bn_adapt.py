...
class AdaptiveBatchNorm(nn.Module):
    ...
    def forward(self, input):
        # self.norm(input)
        b,c,_,_ = input.size()
        sigma2,mu=torch.var_mean(input, dim=[2, 3], keepdim=True, unbiased=True)
        running_mean = (
            self.prior * self.layer.running_mean.view(1,c,1,1)
            + (1 - self.prior) * mu.view(b,c,1,1)
        )
        running_var = (
            self.prior * self.layer.running_var.view(1,c,1,1)
            + (1 - self.prior) * sigma2.view(b,c,1,1)
        )
        
        x = self.layer.weight.view(1, -1, 1, 1) * (input - running_mean) * torch.rsqrt(running_var + self.layer.eps) + self.layer.bias.view(1, -1, 1, 1)
