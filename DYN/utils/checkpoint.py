...
def init_checkpoint(conf: Any):
    ...
    conf.checkpoint_path = os.path.join(
        ...
        # f"threhold{conf.threhold_datta}_{conf.model_name}_{conf.base_data_name}_{conf.model_adaptation_method}_{conf.model_selection_method}_{str(time.time()).replace('.', '_')}-seed{conf.seed}",
    )
